...
def line_intersects_circle(x1, y1, x2, y2, a, b, r):
    ...
    if (x_intersect >= min(x1, x2) and x_intersect <= max(x1, x2)) and (
        y_intersect >= min(y1, y2) and y_intersect <= max(y1, y2)
    ):
        return True
    else:
        return False
...
def build_rrt(start, goal, iterations, max_dist, circle_obstacles):
    ...
    for i in range(iterations):
            ...
            if not check_in_circle(candidate1, circle_obstacles):
                sorted_list = sorted(
                    ...
                    key=lambda x: math.sqrt(
                        (candidate1[0] - x[0]) ** 2 + (candidate1[1] - x[1]) ** 2
                    ),
                )
        ...
        while True:
            ...
            if not check_in_circle(candidate2, circle_obstacles):
                sorted_list = sorted(
                    node_dict2.keys(),
                    key=lambda x: math.sqrt(
                        (candidate2[0] - x[0]) ** 2 + (candidate2[1] - x[1]) ** 2
                    ),
                )
        ...
        for i in node_list1:
            sorted_node_list2 = sorted(
                node_list2,
                key=lambda a: math.sqrt((i[0] - a[0]) ** 2 + (i[1] - a[1]) ** 2),
            )

            for k in sorted_node_list2:
                if (
                    check_connectable(i, k, circle_obstacles)
                    and math.sqrt((k[0] - i[0]) ** 2 + (k[1] - i[1]) ** 2) < max_dist
                ):
